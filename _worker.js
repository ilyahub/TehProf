export default {
  async fetch(request) {
    ...
    if (!sdk) {
      // минимальный «страховочный» вариант: если BX24 не подложен порталом
      sdk =
    ...
    // ——— Весь HTML/JS. Внутри нет «вложенных» template-строк — только обычные кавычки.
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .wrap-title a{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:inline-block;max-width:420px}
</style>
</head><body>
  <h1>Лицензии</h1>
  ...
  (function(){
    ...
    var S = {
      ...
      cols: JSON.parse(localStorage.getItem('cols_v1')||'null') || ['stage','deal','key','url','tariff','tEnd','mEnd','product','act'],
    ...
    BX24.init(function(){
      ...
      BX24.placement.info(function(){ start(); }); setTimeout(start,300); setTimeout(start,1500);
    });
    ...
    return new Response(html, {
      headers: {
        ...
        'content-security-policy': `default-src 'self' data: blob:; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src *; frame-ancestors ${PORTAL_ORIGIN} https://*.bitrix24.kz;`
      }
    });
  }
};
